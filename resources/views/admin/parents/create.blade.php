<x-layouts::app :title="__('Add Parent')">
    <div class="space-y-6">
        <x-admin-header :title="__('Add Parent')" />
        ...
        <div class="max-w-2xl rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6">
            <form method="POST" action="{{ route('admin.parents.store') }}" class="space-y-6">
                ...
                <flux:input name="occupation" :label="__('Occupation')" :value="old('occupation')" />

                <flux:textarea name="address" :label="__('Address')" rows="2">{{ old('address') }}</flux:textarea>

                <fieldset>
                    <legend class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">{{ __('Link to Students') }} <span class="text-red-500">*</span></legend>
                    @if ($students->count())
                    ...
                    @else
                        <p class="text-sm text-zinc-500">{{ __('No active students found. Create students first.') }}</p>
                    @endif
                    @error('student_ids')
                ...
                <div class="flex gap-3">
                    <flux:button variant="primary" type="submit">{{ __('Add Parent') }}</flux:button>
