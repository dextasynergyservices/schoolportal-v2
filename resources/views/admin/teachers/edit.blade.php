    ...
    <div class="space-y-6">
        ...
        <div class="max-w-2xl rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6">
            <form method="POST" action="{{ route('admin.teachers.update', $teacher) }}" class="space-y-6">
                ...
                <flux:select name="level_id" :label="__('School Level')">
                    <option value="">{{ __('All levels') }}</option>
                    @foreach ($levels as $level)
                        <option value="{{ $level->id }}" @selected(old('level_id', $teacher->level_id) == $level->id)>{{ $level->name }}</option>
                    @endforeach
                </flux:select>
                    ...
                    <fieldset>
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
                            @foreach ($classes as $class)
                                <flux:checkbox name="class_ids[]" :value="$class->id" :label="$class->name" :checked="in_array($class->id, old('class_ids', $assignedClassIds))" />
                            @endforeach
                        </div>
                    </fieldset>
                @endif
