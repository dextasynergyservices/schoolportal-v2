<x-layouts::app :title="__('Edit Subject')">
    <div class="space-y-6">
        <x-admin-header :title="__('Edit Subject: :name', ['name' => $subject->name])" />
        ...
        <div class="max-w-xl rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6">
            <form method="POST" action="{{ route('admin.subjects.update', $subject) }}" class="space-y-6">
                ...
                <flux:input name="name" :label="__('Subject Name')" :value="old('name', $subject->name)" required />

                <div class="grid grid-cols-2 gap-4">
                    <flux:input name="short_name" :label="__('Short Name')" :value="old('short_name', $subject->short_name)" placeholder="e.g. MATH, ENG" />
                    <flux:input name="category" :label="__('Category')" :value="old('category', $subject->category)" placeholder="e.g. Science, Arts" />
                </div>

                <flux:input name="sort_order" :label="__('Sort Order')" :value="old('sort_order', $subject->sort_order)" type="number" min="0" />

                <flux:switch name="is_active" :label="__('Active')" :checked="old('is_active', $subject->is_active)" value="1" />
                ...
                @if($classes->isNotEmpty())
                    <div
                        x-data="{
                            search: '',
                            open: false,
                            selected: @js(old('class_ids', $assignedClassIds)).map(Number),
                    ...
                    >
                        ...
                        <flux:description class="mt-1">{{ __('Select which classes offer this subject.') }}</flux:description>
                        @error('class_ids')
                ...
                <div class="flex gap-3">
                    <flux:button variant="primary" type="submit">{{ __('Update Subject') }}</flux:button>
