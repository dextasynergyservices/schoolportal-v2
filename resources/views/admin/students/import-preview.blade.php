<x-layouts::app :title="__('Import Preview')">
    <div class="space-y-6">
        <x-admin-header :title="__('Import Preview')">
            <flux:button variant="subtle" size="sm" href="{{ route('admin.students.import') }}" wire:navigate icon="arrow-left">
                {{ __('Back') }}
            </flux:button>
        </x-admin-header>
        ...
        @if ($validCount > 0)
            <div class="flex gap-3">
                ...
                <flux:button variant="ghost" href="{{ route('admin.students.import') }}" wire:navigate>{{ __('Cancel') }}</flux:button>
            </div>
            @if ($errorCount > 0)
