<x-layouts::app :title="__(':name — :term Result', ['name' => $child->name, 'term' => $result->term?->name])">
    <div class="space-y-6">
        {{-- Breadcrumb --}}
        <div class="flex items-center gap-2 flex-wrap">
            ...
            <flux:icon.chevron-right class="w-3 h-3 text-zinc-400" />
            <flux:link href="{{ route('parent.children.results', $child) }}" wire:navigate class="text-sm text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300">
                {{ __('Results') }}
            </flux:link>
            <flux:icon.chevron-right class="w-3 h-3 text-zinc-400" />
        ...
        <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 overflow-hidden">
            <div class="aspect-[3/4] sm:aspect-[4/5] md:aspect-[16/10] w-full">
                <iframe
                    ...
                    class="w-full h-full"
                    title="{{ __('Result PDF for :name — :session :term', ['name' => $child->name, 'session' => $result->session?->name, 'term' => $result->term?->name]) }}"
                    loading="lazy"
                ></iframe>
            </div>
        </div>
