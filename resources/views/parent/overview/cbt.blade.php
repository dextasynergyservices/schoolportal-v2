<x-layouts::app :title="__('CBT Results')">
    <div class="space-y-6">
        <x-admin-header
            ...
            :description="__('View your children\'s CBT exam, assessment, and assignment results')"
        />
                    ...
                    <a href="{{ route('parent.children.cbt-results', $child) }}" wire:navigate
                       class="group rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 hover:border-indigo-300 dark:hover:border-indigo-600 transition-all hover:shadow-md overflow-hidden">
                        <div class="p-5">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <div class="rounded-md bg-zinc-50 dark:bg-zinc-700/30 px-3 py-2 text-center">
                                    ...
                                    <p class="text-lg font-bold text-zinc-900 dark:text-white">{{ $child->stat_cbt_taken }}</p>
                                </div>
                                <div class="rounded-md bg-zinc-50 dark:bg-zinc-700/30 px-3 py-2 text-center">
                                    <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('Passed') }}</p>
                                    <p class="text-lg font-bold text-green-600 dark:text-green-400">{{ $child->stat_cbt_passed }}</p>
                                </div>
                                <div class="rounded-md bg-zinc-50 dark:bg-zinc-700/30 px-3 py-2 text-center">
                                    <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('Avg') }}</p>
                                    <p class="text-lg font-bold text-indigo-600 dark:text-indigo-400">
                                        {{ $child->stat_cbt_avg !== null ? $child->stat_cbt_avg . '%' : '—' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="px-5 py-2.5 border-t border-zinc-100 dark:border-zinc-700/50 bg-zinc-50/50 dark:bg-zinc-800/50">
                            <span class="text-xs font-medium text-indigo-600 dark:text-indigo-400 group-hover:underline">{{ __('View CBT results') }} &rarr;</span>
                        </div>
                    </a>
                @endforeach
            </div>
