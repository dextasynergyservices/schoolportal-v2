<x-layouts::app :title="__('Games')">
    <div class="space-y-6">
        <x-admin-header
            :title="__('Games')"
            :description="__('View your children\'s educational game progress')"
        />
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($children as $child)
                    <a href="{{ route('parent.children.games', $child) }}" wire:navigate
                       class="group rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 hover:border-emerald-300 dark:hover:border-emerald-600 transition-all hover:shadow-md overflow-hidden">
                        <div class="p-5">
                            {{-- Child Info --}}
                            <div class="flex items-center gap-3 mb-4">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="font-semibold text-zinc-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors truncate">
                                        {{ $child->name }}
                                    </p>
                                ...
                                <flux:icon.chevron-right class="w-5 h-5 text-zinc-400 group-hover:text-emerald-500 transition-colors shrink-0" />
                            </div>

                            {{-- Stats --}}
                            <div class="grid grid-cols-3 gap-2">
                                <div class="rounded-md bg-zinc-50 dark:bg-zinc-700/30 px-3 py-2 text-center">
                                    <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('Played') }}</p>
                                    <p class="text-lg font-bold text-zinc-900 dark:text-white">{{ $child->stat_games_played }}</p>
                                </div>
                                <div class="rounded-md bg-zinc-50 dark:bg-zinc-700/30 px-3 py-2 text-center">
                                    <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('Avg') }}</p>
                                    <p class="text-lg font-bold text-emerald-600 dark:text-emerald-400">
                                        {{ $child->stat_game_avg !== null ? $child->stat_game_avg . '%' : '—' }}
                                    </p>
                                </div>
                                <div class="rounded-md bg-zinc-50 dark:bg-zinc-700/30 px-3 py-2 text-center">
                                    <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('Best') }}</p>
                                    <p class="text-lg font-bold text-zinc-900 dark:text-white">
                                        {{ $child->stat_game_best !== null ? $child->stat_game_best . '%' : '—' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="px-5 py-2.5 border-t border-zinc-100 dark:border-zinc-700/50 bg-zinc-50/50 dark:bg-zinc-800/50">
                            <span class="text-xs font-medium text-emerald-600 dark:text-emerald-400 group-hover:underline">{{ __('View game stats') }} &rarr;</span>
                        </div>
                    </a>
                @endforeach
            </div>
