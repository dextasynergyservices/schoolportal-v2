<x-layouts::app :title="__('My Games')">
    <div class="space-y-6">
        <x-admin-header
            :title="__('My Games')"
            :action="route('teacher.games.create')"
            :actionLabel="__('Create Game')"
            actionIcon="plus"
        />
        ...
        <form method="GET" action="{{ route('teacher.games.index') }}" class="flex flex-wrap items-end gap-3">
            <div>
            ...
            @if (request()->hasAny(['class_id', 'game_type', 'status']))
                <flux:button variant="subtle" size="sm" href="{{ route('teacher.games.index') }}" wire:navigate>{{ __('Clear') }}</flux:button>
            @endif
        </form>
        ...
        <div class="grid gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                                {{ $game->class?->name }}
                                &middot; {{ $game->gameTypeLabel() }}
                                &middot; {{ ucfirst($game->difficulty) }}
                            </p>
                            <p class="mt-1 text-xs text-zinc-400">
                                {{ $game->session?->name }} / {{ $game->term?->name }}
                                &middot; {{ $game->created_at->format('M j, Y') }}
                            </p>
                        </div>
                        <div class="shrink-0">
                        ...
                        <div class="mt-2 rounded-md bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-3 py-2">
                            <div class="flex items-start gap-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-red-700 dark:text-red-300 mt-0.5">{{ $game->latestTeacherAction->rejection_reason }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                    <div class="mt-3 flex flex-wrap items-center gap-2">
                        <flux:button variant="subtle" size="sm" href="{{ route('teacher.games.show', $game) }}" wire:navigate>{{ __('Preview') }}</flux:button>
                        @if (in_array($game->status, ['draft', 'pending', 'rejected']))
                            <flux:button variant="subtle" size="sm" href="{{ route('teacher.games.edit', $game) }}" wire:navigate>{{ __('Edit') }}</flux:button>
                        @endif
                        @if ($game->is_published)
                            <flux:button variant="subtle" size="sm" href="{{ route('teacher.games.stats', $game) }}" wire:navigate>{{ __('Stats') }}</flux:button>
                        @endif
                        @if (! $game->is_published && $game->status !== 'approved')
                                ...
                                <div x-show="showConfirm" x-cloak x-transition class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showConfirm = false">
                                    <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-xl p-6 max-w-sm mx-4" @click.stop>
                                        ...
                                        <div class="flex justify-end gap-2">
                                            ...
                                            <form method="POST" action="{{ route('teacher.games.destroy', $game) }}" @submit="deleting = true">
                                                @csrf
                                                @method('DELETE')
            ...
            @empty
                <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-8 text-center">
                    ...
                    <p class="mt-1 text-sm text-zinc-500">{{ __('Create your first educational game.') }}</p>
                    <div class="mt-4">
                        <flux:button variant="primary" size="sm" href="{{ route('teacher.games.create') }}" wire:navigate>{{ __('Create Game') }}</flux:button>
                    </div>
                </div>
            @endforelse
        </div>
