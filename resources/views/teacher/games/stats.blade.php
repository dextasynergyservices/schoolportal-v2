<x-layouts::app :title="__('Game Stats')">
    <div class="space-y-6">
        <div>
            ...
            <p class="text-sm text-zinc-500 mt-1">{{ $game->class?->name }} &middot; {{ $game->gameTypeLabel() }} &middot; {{ ucfirst($game->difficulty) }}</p>
        </div>
        ...
        @else
            <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-8 text-center">
                ...
                <p class="mt-1 text-sm text-zinc-500">{{ __('Students haven\'t played this game yet.') }}</p>
            </div>
        @endif
    </div>
</x-layouts::app>
