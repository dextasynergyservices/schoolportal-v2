    ...
    <div class="flex flex-col gap-6 animate-fade-in-up">
        ...
        <div class="flex flex-col items-center gap-3 lg:hidden">
            <a href="{{ route('home') }}" class="inline-flex items-center gap-2.5 no-underline group">
                ...
                <span class="text-lg font-bold tracking-tight text-zinc-900 dark:text-white">SchoolPortal</span>
            </a>
        </div>
        ...
        @if ($school)
            <div class="flex flex-col items-center gap-2 text-center">
                ...
                @endif
                <h2 class="text-xl font-bold text-zinc-900 dark:text-white">{{ $school->name }}</h2>
                @if ($school->motto)
                    <p class="text-sm italic text-zinc-500 dark:text-zinc-400">{{ $school->motto }}</p>
                @endif
                <div class="w-12 h-1 rounded-full bg-[#00b2ff] mt-1"></div>
            </div>
        @else
        ...
        <form method="POST" action="{{ route('login.store') }}" class="flex flex-col gap-5">
            ...
            @if (! $school && app()->environment('local'))
                <flux:input
                    name="school_id"
                    :label="__('School ID')"
                    :value="old('school_id', session('school_id'))"
                    type="number"
                    required
                    placeholder="1"
                />
            ...
            <button type="submit" data-test="login-button" class="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-[#000c99] px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-[#00075d] focus:outline-none focus:ring-2 focus:ring-[#00b2ff] focus:ring-offset-2 transition-colors dark:focus:ring-offset-zinc-900">
                <span>{{ __('Log in') }}</span>
        ...
        {{-- Mobile-only footer --}}
        <p class="text-center text-xs text-zinc-400 dark:text-zinc-500 lg:hidden mt-2">
            &copy; {{ date('Y') }} SchoolPortal
        </p>
    </div>
</x-layouts::auth>
