<x-layouts::app :title="__('My Assignments')">
    <div class="space-y-6">
        <x-admin-header :title="__('My Assignments')" :description="__('View class assignments by session, term, and week.')" />

        {{-- Filters --}}
        <form method="GET" action="{{ route('student.assignments.index') }}" class="flex flex-wrap items-end gap-3">
            <div>
            ...
            @if (request()->hasAny(['session_id', 'term_id', 'week']))
                <flux:button variant="subtle" size="sm" href="{{ route('student.assignments.index') }}" wire:navigate>
                    {{ __('Clear') }}
                </flux:button>
            @endif
        </form>
